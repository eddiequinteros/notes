<html>
<head>
<title>Search Notes By Date Result</title>
...
print "<center><h2>Results on notes search with criteria  </h2>";
...
while ($row = $result->fetchArray())
{
	...
	echo "<tr>
	<td><a target=_blank href=list_note_number.php?id={$row['id']}>{$row['id']}</a></td>
	<td>{$row['Vendor']}</td>
	<td>{$row['Component']}</td>
	<td>{$row['Title']}</td></tr>";
}
...
?>
<tr>
 <td><input type="button" value="Close Window" onclick="self.close()"></td>
